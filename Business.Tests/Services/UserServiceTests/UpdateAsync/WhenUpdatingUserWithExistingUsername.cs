...
namespace Business.Tests.Services.UserServiceTests.UpdateAsync
{
    ...
    public class WhenUpdatingUserWithExistingUsername : UserMetadataSearchStrategyTestBase
    {
        ...
        {
            Assert.That(async() => await ClassInTest.UpdateAsync(new User { Id = Guid.NewGuid(), Username = ValidUser.Username }, TestCancellationToken), Throws.InstanceOf<UsernameAlreadyTakenException>());
        }
    }
}